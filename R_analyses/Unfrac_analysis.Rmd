...
```{r, fig.height=2.296587, fig.width=2.296587, message=FALSE, warning=FALSE}
# Get centroids for plotting the paths
unfrac_meta.df = data.frame(sample_data(unfrac.physeq)) %>%
  mutate(treatment = ifelse(substrate == "H2O-Con", "Water", "Carbon")) %>%
  mutate(group = paste(ecosystem, day, treatment, sep="-"))
...
Plot all together
```{r, fig.height=3, fig.width=7.08661, message=FALSE, warning=FALSE}
library(grid)
library(gridExtra)
...
```{r, fig.height=4.72441, fig.width=6.69291, message=FALSE, warning=FALSE}
...
```{r, fig.height=4, fig.width=7.08661, message=FALSE, warning=FALSE}
# Set colors for the taxa
source("/Users/sambarnett/Documents/Misc_code/paul_tol_colors.R")
taxa = unique(OTU_table.taxa.meta.df[OTU_table.taxa.meta.df$taxa != "Less than 1%",]$taxa)
taxa.cols = c(paultol_colors(length(taxa)), "#777777")
...
```{r, fig.height=7.08661, fig.width=7.08661, message=FALSE, warning=FALSE}
...
## DNA concentration
```{r, fig.height=3.34646, fig.width=5, message=FALSE, warning=FALSE}
