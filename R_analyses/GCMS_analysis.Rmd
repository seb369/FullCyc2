...
# Do any samples have a calculated CO2 concentration below 0.
CO2_conc.df[CO2_conc.df$CO2_ppm < 0,]
...
For my first analysis I want to see if the total carbon mineralization rates differe between land use regimes over time. I will do this analysis using a linear mixed effects model. For this analysis we can use all samples from a land-use (except for the H2O-Con) as replicates since they were all treated the same except for the 13C label.
...
```{r, fig.height=3.34646, fig.width=6.69291}
...
```{r, fig.height=6.69291, fig.width=3.46457}
# Run the linear mixed effects model for each substrate individually
sub.model.list = list()
sub.rate_lme.df = data.frame()
sub.rate_means.df = data.frame()
...
```{r, fig.width=3.34646, fig.height=3.93701}
...
```{r, fig.height=6.69291, fig.width=3.34646}
# Get the day 14 and 30 cummulative total carbon mineralization values
GCMS.df = read.table(file="/Users/sambarnett/Documents/Buckley Lab/FullCyc2/Analysis/GCMS_calc_data.txt",
                     header = TRUE, sep="\t", stringsAsFactors = FALSE)
