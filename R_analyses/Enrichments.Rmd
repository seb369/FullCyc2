...
```{r, fig.height=7.08661, fig.width=3.46457}
...
Now I want to see if I can identify any OTUs that are significantly enriched in the treated samples compared to the control samples.
...
```{r}
enr.deseq.df %>% 
  filter(padj < 0.05, OTU %in% unique(filter(l2fc.df, padj < 0.05)$OTU)) %>%
  group_by(OTU) %>%
  summarize(n = n())

incorp.enr.deseq.df = l2fc.df %>%
  filter(padj < 0.05) %>%
  select(OTU, substrate, ecosystem, Domain, Phylum, Class, Order, Family, Genus, Species) %>%
  unique %>%
...
incorp.enr.deseq.df %>%
  group_by(OTU, Domain, Phylum, Class, Order, Family, Genus, Species) %>%
  summarize(n_times = n())
